...
public class MainActivity extends AppCompatActivity {
    ...
    private View.OnClickListener listener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            String limitString = search.getText().toString();
            int limit = 0;
            try {
                limit = Integer.parseInt(limitString);
            }
            catch (NumberFormatException exception) {
                output.setText("Введенное выражение не является целым числом!");
                return;
            }
            mapPoem = stringMapPoem(poem);

            String message = getStringsCount(limit);
            if (message == null) output.setText("Данной строки поиска в поэме не найдено!");
            else output.setText("В поэме найдено " + message);
        }
    };

    @Nullable
    private String getStringsCount(int limit) {
        int count = 0;
        String result = "";

        for (int i = 0; i < mapPoem.size(); i++) {
            String object = mapPoem.get(i);

            if (!(object.length() == limit)) continue;
            else {
                count++;
                result = result + "\n\n" + object;
            }
        ...
        if (count == 0) {
            ...
            return count + " строк с введённой длиной строки.\nСтроки:" + result;
        }
    }
    ...
    private String searchMapPoem(String search, Map<Integer, String> map) {
        ...
        for (Integer key: map.keySet()) { // перебор мап по ключам
            if(map.get(key).contains(search)) { // если в строке с ключом key имеется фраза поиска, то
